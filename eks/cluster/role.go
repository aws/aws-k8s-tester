...
func (ts *tester) createPolicy() error {
	if ts.cfg.EKSConfig.Role.PolicyName == "" {
...
func (ts *tester) attachPolicy() error {
	...
	if err != nil {
		ts.cfg.Logger.Warn("failed to attach policy", zap.String("arn", ts.cfg.EKSConfig.Role.PolicyARN), zap.Error(err))
		return err
	}
	ts.cfg.Logger.Info("attached policy arn", zap.String("policy-arn", ts.cfg.EKSConfig.Role.PolicyARN))

	for _, arn := range ts.cfg.EKSConfig.Role.ManagedPolicyARNs {
...
func (ts *tester) detachPolicy() error {
	...
	if err != nil {
		ts.cfg.Logger.Warn("failed to detach policy", zap.String("arn", ts.cfg.EKSConfig.Role.PolicyARN), zap.Error(err))
		return err
	}
	for _, arn := range ts.cfg.EKSConfig.Role.ManagedPolicyARNs {
		time.Sleep(3 * time.Second)
