...
func (ts *tester) checkECR() error {
	...
	if len(out.Repositories) != 1 {
		return fmt.Errorf("%q expected 1 ECR repository, got %d", ts.cfg.EKSConfig.AddOnHollowNodesRemote.RepositoryName, len(out.Repositories))
	}
	...
	if name != ts.cfg.EKSConfig.AddOnHollowNodesRemote.RepositoryName {
		return fmt.Errorf("unexpected ECR repository name %q", name)
	}
	if uri != ts.cfg.EKSConfig.AddOnHollowNodesRemote.RepositoryURI {
	...
	imgOut, err := ts.cfg.ECRAPI.DescribeImages(&ecr.DescribeImagesInput{
		...
		ImageIds: []*ecr.ImageIdentifier{
			{
				ImageTag: aws.String(ts.cfg.EKSConfig.AddOnHollowNodesRemote.RepositoryImageTag),
			},
		},
	})
	if err != nil {
	...
	if len(imgOut.ImageDetails) == 0 {
		return fmt.Errorf("image tag %q not found", ts.cfg.EKSConfig.AddOnHollowNodesRemote.RepositoryImageTag)
	}
	...
	for i, img := range imgOut.ImageDetails {
		ts.cfg.Logger.Info("found an image",
			zap.Int("index", i),
			zap.String("requested-tag", ts.cfg.EKSConfig.AddOnHollowNodesRemote.RepositoryImageTag),
