// k8s-tester-csi-efs installs Kubernetes csi-efs tester.
package main
...
var rootCmd = &cobra.Command{
	Use:        "k8s-tester-csi-efs",
	Short:      "Kubernetes CSI EFS tester",
	SuggestFor: []string{"csi-efs"},
}
...
func init() {
	...
	rootCmd.PersistentFlags().IntVar(&minimumNodes, "minimum-nodes", csi_efs.DefaultMinimumNodes, "minimum number of Kubernetes nodes required for installing this addon")
...
func main() {
	if err := rootCmd.Execute(); err != nil {
		fmt.Fprintf(os.Stderr, "k8s-tester-csi-efs failed %v\n", err)
		os.Exit(1)
	}
	os.Exit(0)
}
...
func newApply() *cobra.Command {
	...
	cmd.PersistentFlags().StringVar(&helmChartRepoURL, "helm-chart-repo-url", csi_efs.DefaultHelmChartRepoURL, "helm chart repo URL")
	return cmd
}
...
func createApplyFunc(cmd *cobra.Command, args []string) {
	...
	fmt.Printf("\n*********************************\n")
	fmt.Printf("'k8s-tester-csi-efs apply' success\n")
}
...
func createDeleteFunc(cmd *cobra.Command, args []string) {
	...
	fmt.Printf("\n*********************************\n")
	fmt.Printf("'k8s-tester-csi-efs delete' success\n")
}
