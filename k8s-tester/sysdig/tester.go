// package sysdig installs Sysdig agent.
// ref. https://sysdig.org
package sysdig
...
func (cfg *Config) ValidateAndSetDefaults() error {
	...
	if cfg.AccessKey == "" {
		return errors.New("empty Sysdig Access Token")
	}
	if cfg.CollectorEndpoint == "" {
		return errors.New("empty Sysdig Collector Endpoint")
	}
	return nil
}

const chartName = "sysdig"

const (
	DefaultMinimumNodes     int = 1
	DefaultHelmChartRepoURL     = "https://github.com/sysdiglabs/charts/releases/download/sysdig-1.12.9/sysdig-1.12.9.tgz"
)
...
func (ts *tester) Apply() error {
	...
	if err := ts.createHelmSysdig(); err != nil {
		return err
	}
...
func (ts *tester) Delete() error {
	...
	if err := ts.deleteHelmSysdig(); err != nil {
...
// https://github.com/sysdiglabs/charts/blob/master/charts/sysdig/values.yaml
func (ts *tester) createHelmSysdig() error {
	values := map[string]interface{}{
		"sysdig": map[string]interface{}{
			"accessKey": ts.cfg.AccessKey,
		},
		"nodeAnalyzer": map[string]interface{}{
			"collectorEndpoint": ts.cfg.CollectorEndpoint,
		},
	}
	...
	descArgsPods := []string{
		...
		"describe",
		"pods",
		"--selector=app.kubernetes.io/name=sysdig",
	}
	...
	logArgs := []string{
		...
		"--namespace=" + ts.cfg.Namespace,
		"logs",
		"--selector=app=app.kubernetes.io/name=sysdig",
		"--all-containers=true",
		"--timestamps",
	}
	...
	return helm.Install(helm.InstallConfig{
		...
		QueryFunc: func() {
			...
			if err != nil {
				...
				ts.cfg.Logger.Warn("Sysdig Tests::", zap.String("TEST", "FAILED"))
			}
...
func (ts *tester) deleteHelmSysdig() error {
