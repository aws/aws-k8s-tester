...
def create_dummy_data(tokenizer, num_samples=104, max_length=128):
    ...
    return TensorDataset(
        input_ids, tokenized_inputs.attention_mask
    )
...
def run_inference(model, tokenizer, batch_size, mode):
    ...
    model_neuron = torch_neuronx.trace(model, (inputs, masks))
    # # Compile the model to neuron device
    filename = 'model.pt'
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            outputs = model_neuron(inputs, masks)
            end_time = time.time()
