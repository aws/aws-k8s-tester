...
def create_dummy_data(tokenizer, num_samples=100, max_length=128):
    ...
    sentences = ["This is a dummy sentence number {}".format(i) for i in range(num_samples)]
    tokenized_inputs = tokenizer(sentences, max_length=max_length, padding="max_length", truncation=True, return_tensors="pt")
...
def train_bert(rank, world_size, model, tokenizer):
    ...
    for epoch in range(1):  # Short run for testing
        ...
        for batch in train_dataloader:
            ...
            inputs, masks, labels, next_sentence_labels = inputs.to(rank), masks.to(rank), labels.to(rank), next_sentence_labels.to(rank)
            outputs = ddp_model(input_ids=inputs, attention_mask=masks, labels=labels, next_sentence_label=next_sentence_labels)
            loss = outputs.loss
            loss.backward()
            optimizer.step()
