...
/*
remotePath := fmt.Sprintf("/home/%s/private.key", ec2Config.UserName)
	_, err = ss.Send(
		privateKeyPath,
		remotePath,
		ssh.WithTimeout(15*time.Second),
	...
	copyCmd := fmt.Sprintf("sudo mkdir -p %s && sudo cp %s %s", remotePath, remotePath)
	_, err = ss.Run(
		copyCmd,
		ssh.WithTimeout(15*time.Second),
		ssh.WithRetry(3, 3*time.Second),
	...
	remotePath = fmt.Sprintf("/home/%s/public.pem", ec2Config.UserName)
	_, err = ss.Send(
		publicKeyPath,
		remotePath,
		ssh.WithTimeout(15*time.Second),
