...
type VPC struct {
	...
	PublicSubnetCIDRs                    []string `json:"public-subnet-cidrs,omitempty"`
	...
	DHCPOptionsID                string   `json:"dhcp-options-id" read-only:"true"`
}
...
func (cfg *Config) validateParameters() error {
	...
	case false: // use existing one
		...
		if len(cfg.Role.ManagedPolicyARNs) > 0 {
			return fmt.Errorf("Role.Create false; expect empty RoleManagedPolicyARNs but got %q", cfg.Role.ManagedPolicyARNs)
		}
		if len(cfg.Role.ServicePrincipals) > 0 {
			return fmt.Errorf("Role.Create false; expect empty RoleServicePrincipals but got %q", cfg.Role.ServicePrincipals)
		}
	}
	...
	case false: // use existing one
		if cfg.Encryption.CMKARN == "" {
			// return fmt.Errorf("Parameters.EncryptionCMKCreate false; expect non-empty EncryptionCMKARN but got %q", cfg.Encryption.CMKARN)
		}
	}
